# 司法主观题考试
## 任务介绍
司法主观题考试通常是法律职业资格考试中的重要部分，其主要目的是评估考生的法律理论知识、法律逻辑推理能力和实际问题解决能力。司法主观题包含多种题型（论述题、案例分析题、法律文书题等题型）。本任务聚焦在论述题和案例分析题，要求参赛选手的答案结论正确且尽可能命中采分点。
...
| 字段名          | 数据类型  | 描述                                         |
|----------------|----------|---------------------------------------------|
| id             | str      | 编号（初赛0/复赛1/封赛2_训练0/测试1_序号）       |
...
## 评价方式
1. 本任务在初赛和复赛阶段将使用大模型自动评价，在封赛阶段将使用大模型+人工的均值评价。
2. 评价指标为得分率（指实际得分user_score除以试题总分total_score的比值，换算成的百分数），公式定义如下：scoring_rate= user_score / total_score*100%。
3. 最终成绩由初赛阶段、复赛阶段和封赛阶段的成绩综合决定：score = 0.1 * scoring_rate_初赛 + 0.3 * scoring_rate_复赛 + 0.6 * scoring_rate_封测。

**注意**
1. 答案的结论错，即认为全错，考题得分为0分。
2. 要求答案尽可能简短，将以标准答案长度的两倍进行截取。

## 结果提交形式
1. 在初赛和复赛阶段，本任务的评测方式为自动评测，需在线提交测试集上生成结果的json文件。在封赛阶段，将以最后一次提交的生成结果的json文件，进行自动评测（立即给出）+人工评测（延后给出）。
2. 提交结果统一命名为prediction.json，其中每一行对应一道试题的答案（注意：id必须与测试集对应）。
...
## 基线系统
本赛道采用qwen3-32b作为基线模型。`baseline.py`中提供了一个基于`Qwen`的代码实现，输入格式参考`test_data.json`，输出格式参考`prediction.json`
...
3. 封赛阶段（11月20日-11月30日）：封赛评测。我们将提供新测试集并开放第三阶段的有限次数的提交。